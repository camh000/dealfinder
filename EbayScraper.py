...
def _category_for_ebay_id(cur, ebay_id) -> str | None:
    """Which category satellite table holds this listing (for parsing)."""
    for cat in queries.CATEGORIES:
        cur.execute(
            f"SELECT 1 FROM Scraper.{queries.CATEGORIES[cat]['table']} WHERE ID = %s LIMIT 1",
            (ebay_id,))
        if cur.fetchone():
            return cat
    return None
...
def VerifyBinAvailability(max_check: int = 40, give_up_days: int = 14) -> dict:
    """Resolve surfaced Buy-It-Now finds that dropped out of the active feed.

    A BIN has no end time, so it can sell (or be pulled) at any moment and the
    only signal is that it stops appearing in the active sweeps — its LastSeenAt
    goes stale. For each surfaced BIN still marked unsold that has gone stale,
    look it up by ID in eBay's SOLD results: a hit means it sold, so stamp
    SoldDate + the sold price. If it's not sold but still shows in ACTIVE results
    it just wasn't in our keyword sweeps — refresh LastSeenAt so it stays in the
    feed. Anything in neither is left for the 14-day stale prune.

    Only STALE finds are checked, so the fetch cost is bounded to listings that
    actually disappeared. Returns {'checked', 'sold', 'still_live'}.
    """
    ...
    try:
        cur.execute(f"""
            SELECT b.EbayID
            FROM   Scraper.BinNotified b
            JOIN   Scraper.EBAY e ON e.ID = b.EbayID
            WHERE  e.SoldDate IS NULL
              ...
              AND  e.LastSeenAt > NOW() - INTERVAL %s DAY
            ORDER BY e.LastSeenAt DESC
            LIMIT  %s
        """, (give_up_days, max_check))
        ids = [r[0] for r in cur.fetchall()]
        if not ids:
            return {'checked': 0, 'sold': 0, 'still_live': 0}

        sold = still_live = 0
        for ebay_id in ids:
            category = _category_for_ebay_id(cur, ebay_id)
            if category is None:
                continue
            # Sold search by ID — a match is conclusive (no two-strike needed;
            # we only ever WRITE a sale we positively found).
            item = _scrape_item_by_id(ebay_id, category, sold=True)
            if item and item.get('sold-date'):
                cur.execute("""
                    UPDATE Scraper.EBAY SET SoldDate = %s, Price = %s
                    WHERE ID = %s AND SoldDate IS NULL
                """, (item['sold-date'], int(round(item['price'] * 100)), ebay_id))
                conn.commit()
                sold += 1
                log.info("BIN verified sold: ID=%s for £%.2f on %s",
                         ebay_id, item['price'], item['sold-date'])
                continue
            # Not sold — still on eBay? Refresh so an available BIN that just
            # fell out of our keyword sweeps doesn't vanish from the feed.
            if _scrape_item_by_id(ebay_id, category, sold=False):
                cur.execute("UPDATE Scraper.EBAY SET LastSeenAt = NOW() WHERE ID = %s", (ebay_id,))
                conn.commit()
                still_live += 1
            # else: removed — leave for the 14-day stale prune.
        log.info("BIN availability: checked %d stale find(s) — %d sold, %d still live",
                 len(ids), sold, still_live)
        return {'checked': len(ids), 'sold': sold, 'still_live': still_live}
    except Exception as e:
        log.error("VerifyBinAvailability failed: %s", e)
        return {'checked': 0, 'sold': 0, 'still_live': 0}
    finally:
        conn.close()
